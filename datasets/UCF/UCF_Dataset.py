...
class UCF_Dataset():
    ...
    def __init__(self, config, annotation_file, data=None, mode='train'):
        ...
        if self.this_data is None:
            ...
            with open(os.path.join(self.config.dataset.data_roots,'UCF_fps1_audio16000_total_data.pkl'), 'rb') as handle:
...
class UCF_Dataloader():
    ...
    def _get_datasets(self):
        annotation_folder = "./datasets/UCF/ucfTrainTestlist"
        # train_dataset = UCF_Dataset(config=self.config, mode="train", annotation_file= os.path.join(annotation_folder, "trainlist01.txt"))
        train_dataset = UCF_Dataset(config=self.config, mode="train", annotation_file=os.path.join(annotation_folder,"trainlist0{}.txt".format(int(self.config.dataset.fold))))
        val_dataset = UCF_Dataset(config=self.config, mode="val", data=train_dataset.this_stored_data,
        ...
        # val_dataset = UCF_Dataset(config=self.config, mode="val", annotation_file=os.path.join(os.path.join(self.config.dataset.data_roots, "ucfTrainTestlist"),"trainlist01.txt"))
        # val_dataset = UCF_Dataset(config=self.config, mode="test", annotation_file= os.path.join(annotation_folder, "testlist01.txt"))
        # val_dataset = UCF_Dataset(config=self.config, mode="val", annotation_folder=os.path.join( self.config.dataset.data_roots, "ucfTrainTestlist"))
        test_dataset = UCF_Dataset(config=self.config, mode="test", annotation_file=os.path.join(annotation_folder,"testlist0{}.txt".format(int(self.config.dataset.fold))))
