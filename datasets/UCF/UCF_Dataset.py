...
class UCF_Dataset:
    ...
    def split_dataset(self, mode):
        ...
        X_train, X_val, y_train, y_val = train_test_split(
            ids, labels, test_size=self.config.dataset.get("val_split_rate", 0.1), random_state=self.config.training_params.seed, stratify=labels
        )
        if mode == "train":
            split_data = {}
...
class UCF_Dataloader:
    ...
    def _get_datasets(self):
        ...
        train_dataset = UCF_Dataset(
            config=self.config, mode="train", annotation_file=os.path.join(annotation_folder, "trainlist0{}.txt".format(int(self.config.dataset.fold)))
        )
        ...
        test_dataset = UCF_Dataset(
            config=self.config, mode="test", annotation_file=os.path.join(annotation_folder, "testlist0{}.txt".format(int(self.config.dataset.fold)))
        )
