...
class MCR_Model(nn.Module):
    def __init__(self, args, encs):
        ...
        if self.cls_type == "linear":
            ...
            self.common_fc = nn.Sequential(
                nn.ReLU(), nn.Dropout(dropout), nn.Linear(fc_inner, fc_inner), nn.ReLU(), nn.Dropout(dropout), nn.Linear(fc_inner, num_classes)
            )
    ...
    def _forward_main(self, a, v, pred_aa, pred_vv, **kwargs):
        ...
        if self.cls_type == "linear" or self.cls_type == "highlynonlinear" or self.cls_type == "nonlinear":
            ...
            if self.training and not kwargs.get("notwandb", False):
                wandb.log(
                    {"wf_a": pred_aa.norm(), "wf_v": pred_vv.norm(), "f_a": a["features"]["combined"].norm(), "f_v": v["features"]["combined"].norm()},
                    step=self.count_trainingsteps + 1,
                )
                self.count_trainingsteps += 1
    ...
    def forward(self, x, **kwargs):
        ...
        output = {"preds": {"combined": pred, "c": pred_aa, "g": pred_vv}, "features": {"c": a["features"]["combined"], "g": v["features"]["combined"]}}
...
class Base_Model(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        if self.mmcosine:
            pred_a = torch.mm(
                F.normalize(a["features"]["combined"], dim=1), F.normalize(torch.transpose(self.fc_0_lin.weight, 0, 1), dim=0)
            )  # w[n_classes,feature_dim*2]->W[feature_dim, n_classes], norm at dim 0.
        ...
        return {"preds": {"combined": pred, "c": pred_aa, "g": pred_vv}, "features": {"c": a["features"]["combined"], "g": v["features"]["combined"]}}
...
class AGM_Model(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        return {
            "preds": {"combined": pred_a + pred_v, "both": pred, "c": pred_a, "g": pred_v},
            "features": {"c": a["features"]["combined"], "g": v["features"]["combined"]},
        }
...
class Base_3D_Model(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        if self.mmcosine:
            pred_a = torch.mm(
                F.normalize(feat_a, dim=1), F.normalize(torch.transpose(self.fc_0_lin.weight, 0, 1), dim=0)
            )  # w[n_classes,feature_dim*2]->W[feature_dim, n_classes], norm at dim 0.
        ...
        output = {"preds": {"combined": pred, "c": pred_aa, "g": pred_vv, "f": pred_zz}, "features": {"c": feat_a, "g": feat_v, "f": feat_z}}
        return output
...
class AGM_3D_Model(nn.Module):
    ...
    def classifier(self, features, noaggr_features, pred_aa, pred_vv, pred_zz, return_all=False, **kwargs):
        ...
        if self.cls_type == "linear" or self.cls_type == "highlynonlinear" or self.cls_type == "nonlinear":
            ...
            tf_input = [noaggr_features["c"].permute(1, 2, 0), noaggr_features["g"].permute(1, 2, 0), noaggr_features["flow"].permute(1, 2, 0)]
    ...
    def forward(self, x, **kwargs):
        ...
        if train_flag:
            self.train()
        m_v_out = self.m_v_o(self.m_v(preds, preds_zv, preds_zl, preds_zf, preds_zvl, preds_zvf, preds_zlf, preds_zvlf))
        m_l_out = self.m_l_o(self.m_l(preds, preds_zv, preds_zl, preds_zf, preds_zvl, preds_zvf, preds_zlf, preds_zvlf))
        m_f_out = self.m_f_o(self.m_f(preds, preds_zv, preds_zl, preds_zf, preds_zvl, preds_zvf, preds_zlf, preds_zvlf))
...
class Modality_Text(nn.Module):
    ...
    def forward(self, total_out, pad_visual_out, pad_audio_out, pad_text_out, pad_visual_audio_out, pad_visual_text_out, pad_audio_text_out, zero_padding_out):
