...
from models.VAVL_git.facial_features.utils.ImageDatasetHelpers import point2bbox, bbpoint_warp
...
class FaceVideoDataModule(FaceDataModuleBase):
    ...
    def __init__(self, root_dir, output_dir, processed_subfolder=None, face_detector="fan", face_detector_threshold=0.9, image_size=224, scale=1.25, device=None):
        super().__init__(
            root_dir,
            output_dir,
    ...
    def _get_path_to_sequence_files(self, sequence_id, file_type, method="", suffix=""):
        assert file_type in ["videos", "detections", "landmarks", "segmentations", "emotions", "reconstructions"]
    ...
    def _detect_faces_in_sequence(self, sequence_id):
        ...
        for fid, frame_fname in enumerate(tqdm(range(start_fid, len(frame_list)))):
            ...
            self._detect_faces_in_image_wrapper(
                frame_list, fid, out_detection_folder, out_landmark_folder, out_file, centers_all, sizes_all, detection_fnames_all, landmark_fnames_all
            )

        FaceVideoDataModule.save_detections(out_file, detection_fnames_all, landmark_fnames_all, centers_all, sizes_all, fid)
    ...
    def create_reconstruction_video(
        self, sequence_id, overwrite=False, distance_threshold=0.5, rec_method="emoca", image_type=None, retarget_suffix=None, cat_dim=0, include_transparent=True
    ):
        ...
        vis_fnames = self._get_reconstructions_for_sequence(sequence_id, rec_method=rec_method, retarget_suffix=retarget_suffix, image_type=image_type)
        ...
        for fid in tqdm(range(len(vid_frames))):
            ...
            if writer is None:
                ...
                writer = cv2.VideoWriter(str(outfile), cv2.CAP_FFMPEG, fourcc, fps, (im.shape[1], im.shape[0]), True)
    ...
    def create_reconstruction_video_with_recognition(self, sequence_id, overwrite=False, distance_threshold=0.5):
        ...
        for fid in tqdm(range(len(vid_frames))):
            ...
            if writer is None:
                fourcc = cv2.VideoWriter_fourcc(*"mp4v")
                # outfile = str(vis_folder / "video.mp4")
                writer = cv2.VideoWriter(str(outfile), cv2.CAP_FFMPEG, fourcc, int(self.video_metas[sequence_id]["fps"].split("/")[0]), (im.shape[1], im.shape[0]), True)
            im_cv = cv2.cvtColor(im, cv2.COLOR_RGB2BGR)
            writer.write(im_cv)
    ...
    def _identify_recognitions_for_sequence(self, sequence_id, distance_threshold=None):
        ...
        detection_fnames, landmark_fnames, centers, sizes, embeddings, recognized_detections_fnames = self._gather_detections_for_sequence(
            sequence_id, with_recognitions=True
        )
        ...
        FaceVideoDataModule._save_recognitions(out_file, labels, recognition_indices, recognition_means, recognition_cov, recognition_fnames)
    ...
    @staticmethod
    def _save_annotations(file, detection_fnames, annotations, recognition_labels, discarded_annotations, detection_not_found):
        with open(file, "wb") as f:
    ...
    def _gather_detections(self, with_recognitions=True):
        ...
        for sid in range(self.num_sequences):
            ...
            if with_recognitions:
                detection_fnames, centers, sizes, embeddings, recognized_detections_fnames = self._gather_detections_for_sequence(
                    sid, with_recognitions=with_recognitions
                )
            else:
    ...
    def assign_gt_to_detections_sequence(self, sequence_id):
        ...
        if len(annotations) == 1 and suffs[0] == "":
            ...
            main_valid_detection_list, main_valid_annotation_list, main_discarded_list, main_detection_not_found_list = self._map_detections_to_gt(
                main_detection_file_names, main_annotation_file, anno_type
            )
            ...
            other_valid_detection_list, other_valid_annotation_list, other_discarded_list, other_detection_not_found_list = self._map_detections_to_gt(
                other_detection_file_names, other_annotation_file, anno_type
            )
            ...
            # THE CASE OF TWO ROUGHLY EQUALY DOMINANT DETECTIONS (LEFT and RIGHT)
                ...
                left_valid_detection_list, left_annotation_list, left_discarded_list, left_detection_not_found_list = self._map_detections_to_gt(
                    left_detection_file_names, left_annotation_file, anno_type
                )
                ...
                right_valid_detection_list, right_valid_annotation_list, right_discarded_list, right_detection_not_found_list = self._map_detections_to_gt(
                    right_detection_file_names, right_annotation_file, anno_type
                )
        ...
        FaceVideoDataModule._save_annotations(
            out_file, annotated_detection_fnames, validated_annotations, recognition_label_dict, discarded_annotations, detection_not_found
        )
...
def attach_audio_to_reconstruction_video(input_video, input_video_with_audio, output_video=None, overwrite=False):
    ...
    cmd = "ffmpeg -y -i %s -i %s -c copy -map 0:0 -map 1:1 -shortest %s" % (input_video, input_video_with_audio, output_video)
    os.system(cmd)
...
class TestFaceVideoDM(FaceVideoDataModule):
    ...
    def __init__(
        ...
        super().__init__(self.video_path.parent, output_dir, processed_subfolder, face_detector, face_detector_threshold, image_size, scale, device)
    ...
    def _get_path_to_sequence_files(self, sequence_id, file_type, method="", suffix=""):
        assert file_type in ["videos", "detections", "landmarks", "segmentations", "emotions", "reconstructions", "results"]
        video_file = self.video_list[sequence_id]
        if len(method) > 0:
            file_type += "/" + method
