...
def main():
    root = "project_default_dir/CremaD/CREMA-D_v2/"
    ...
    # loading model
    model = torch.load("./models/VAVL_git/facial_features/enet_b2_8_best.pt")
    ...
    for ldir in tqdm(list_of_dirs, total=len(list_of_dirs)):
        ...
        if ldir.replace(file_format, ".npy") not in videos:
            ...
            dm = TestFaceVideoDM(input_video, output_folder, processed_subfolder=processed_subfolder, face_detector_threshold=0.96, batch_size=20, num_workers=20)
            dm.prepare_data()
            dm.setup()
