...
class Validator_Tester:
    ...
    def validate(self, test_set=False):
        ...
        with torch.no_grad():
            pbar = tqdm(enumerate(this_dataloader), total=len(this_dataloader), desc="Validation", leave=False, disable=True, position=1)
    ...
    def valtest_one_step(self, served_dict):
        ...
        return {"loss": output_losses, "pred": output["preds"], "features": output["features"], "label": label, "process_flag": process_flag}
