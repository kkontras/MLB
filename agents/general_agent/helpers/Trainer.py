...
class Trainer:
    ...
    def train_one_step(self, served_dict, **kwargs):
        ...
        total_loss, output_losses, optstep_done = self.agent.bias_infuser.before_backward(
            total=total_loss, output_losses=output_losses, w_loss=self.w_loss, loss_fun=calculate_loss, data=data, label=label, output=output
        )
        ...
        this_output.update(
            {"loss": output_losses, "pred": {pred: output["preds"][pred].detach() for pred in output["preds"]}, "label": label.detach().to(self.agent.accelerator.device)}
        )
    ...
    def train_one_step_regression(self, served_dict, **kwargs):
        ...
        if "c" in output["preds"] and "g" in output["preds"]:
            if bias_method == "MLB_3d" or bias_method == "MLB_3d_Reg":
                ...
                self.agent.bias_infuser.on_backward_end(
                    label=label.detach().cpu().cpu(), out_mod0=output["preds"]["c"].detach().cpu(), out_mod1=output["preds"]["g"].detach().cpu()
                )
        ...
        total_loss = total_loss.detach()
        output_losses.update({"total": total_loss})
        this_output.update({"loss": output_losses, "pred": {pred: output["preds"][pred].detach() for pred in output["preds"]}, "label": label.detach()})
    ...
    def local_logging(self, batch_idx, end_of_epoch=None):
        ...
        pbar_message = Fore.WHITE + "Training batch {0:d}/{1:d} steps no improve {2:d} with {3:}".format(
            batch_idx, len(self.agent.data_loader.train_loader) - 1, self.agent.logs["steps_no_improve"], mean_batch_loss_message
        )
        return pbar_message
